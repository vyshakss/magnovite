/**
 * MAGNOVITE 2026 — cinematic cosmic scene (vanilla JS + three.js from CDN).
 * Pulsar -> explosion -> wordmark reveal -> butterfly swarm -> permanent dust cloud.
 * Writes --cine / --flash / --reveal / --chrome onto :root every frame.
 */
import * as THREE from "https://unpkg.com/three@0.169.0/build/three.module.js";
import { PARTICLE_VERT, PARTICLE_FRAG, GLOW_VERT, GLOW_FRAG, RING_FRAG } from "./shaders.js";

const BUTTERFLY_TEXTURE = "logos/magnovite.png";

export function startCosmicScene(host) {
  if (!host) return () => {};

  const mobile =
    typeof navigator !== "undefined" &&
    /Android|iPhone|iPad|iPod|webOS|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
  const tier = mobile ? 0 : 1;
  const COUNT = tier === 0 ? 6000 : 10000;
  ...
  const tex = new THREE.TextureLoader().load(BUTTERFLY_TEXTURE, (t) => {
  ...
  const openingLength = () => {
    const el = document.querySelector("[data-cinematic-scroll]");
    if (el) return Math.max(1, el.offsetHeight - window.innerHeight);
    return Math.max(1, window.innerHeight * 2.8);
  };
  ...
  function tick(now) {
    ...
    glow.scale.setScalar(12 + 40 * smoothstep(0.1, 0.34, progress) + 90 * blast1);
    ...
    root.style.setProperty(
      "--reveal",
      (smoothstep(0.39, 0.45, progress) * (1 - smoothstep(0.55, 0.6, progress))).toFixed(4),
    );
    root.style.setProperty("--chrome", smoothstep(0.7, 0.88, progress).toFixed(4));
